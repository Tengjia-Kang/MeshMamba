...
data_root = base_root + '/MeshFile/rgb_texture' # non_texture or rgb_texture
max_faces = 500
...
for path in fpath_data:
    ...
    path_ring = base_root + "/Ring/"+path_ring.split("/")[-3]+"/"+path_ring.split("/")[-1]
    path_sal = path.replace('.obj', '.csv').replace('MeshFile', 'SaliencyMap')
    path_sal = base_root + "/SaliencyMap/"+path_sal.split("/")[-3]+"/"+path_sal.split("/")[-1]
